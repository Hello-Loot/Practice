"""
通过对视频中的帧进行处理和分析，提取脉冲信号，并基于频域分析计算心率检测的统计信息。
"""
import numpy as np
from cdf import CDF
from asf import ASF
...
class Pulse():
    ...
    def calculate_statistics(self, mean_rgb):
        # 获取脉冲信号
        signal = self.get_pulse(mean_rgb)
        ...
        # 进行频域分析
        freq = rfftfreq(len(self.fft_spec), 1. / self.framerate)
        freq_data = np.mean(self.fft_spec, axis=0)

        # 计算高频成分（HF）
        hf_indices = np.where((freq >= 0.15) & (freq <= 0.4))
        hf_power = np.sum(freq_data[hf_indices])

        # 计算低频成分（LF）
        lf_indices = np.where((freq >= 0.04) & (freq <= 0.15))
        lf_power = np.sum(freq_data[lf_indices])

        # 计算总功率（TP）
        tp_power = np.sum(freq_data)

        # 计算LF/HF比率
        lf_hf_ratio = lf_power / hf_power

        return sdnn, rmssd,  hf_power, lf_power, tp_power, lf_hf_ratio
